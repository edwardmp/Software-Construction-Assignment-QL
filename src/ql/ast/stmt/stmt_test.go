...
func TestFormWithNonEmptyContent(t *testing.T) {
    ...
    questionsListExample := []interfaces.Stmt{questionExample}
    stmtListExample := NewStmtList(questionsListExample)
...
func TestIf(t *testing.T) {
    questionExample := NewInputQuestion(expr.NewStringLiteral("What was the selling price?"), vari.NewVarDecl(vari.NewVarID("sellingPrice"), expr.NewIntegerType()))
    ifBodyExample := NewStmtList([]interfaces.Stmt{questionExample})
...
func TestIfElse(t *testing.T) {
    ...
    ifBodyExample := NewStmtList([]interfaces.Stmt{ifQuestionExample})
    ifCondExample := expr.NewBoolLiteral(true)
    ...
    elseBodyExample := NewStmtList([]interfaces.Stmt{elseQuestionExample})
...
func TestStmtList(t *testing.T) {
    ...
    stmtListExample := NewStmtList([]interfaces.Stmt{questionExample, ifExample})

    assert.Equal(t, len(stmtListExample.Questions()), 1)
    assert.Equal(t, len(stmtListExample.Conditionals()), 1)
}
